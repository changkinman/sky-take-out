...
### 1. 数据库初始化
1.  登录 MySQL。
2.  执行 `docs/sql/sky_take_out.sql` 脚本（如果项目目录下有的话，没有则手动创建库名 `sky_take_out`）。
3.  确保 `sky-server` 中的 `application-dev.yml` 中的数据库用户名和密码与你本地一致。
...
1.  **后端启动**：
    ...
    - 找到 `sky-server` 模块下的 `SkyApplication` 类，直接运行。
2.  **API 调试**：
...
- **Q: 启动报 JWT 签名错误？**
  - A: 确保配置文件中的 `sky.jwt.admin-secret-key` 长度足够（建议不少于 32 位）。
